...
class Benchmark_Multiplicity_Fair:
    ...
    def print_subresults(self, results, metric_names): 
        for name in metric_names:
            ...
            if name == 'avgeo': 
                ...
                line += 'Overall Test Multiplicity:'
            elif name == 'time': 
    ...
    def compute_ave_multiplicity_helper(self, y_pred_list):
        ...
        for eps in params: 
            ...
            for seed in range (num_iter): 
                ...
                for rand in range(rand_seed+1):
                    ### Train rand_seed Models as the Rashomon Set###
                    if model_name == 'gbm': 
                    ...
                    #np.random.seed(rand_seed) # np.random.seed doesn't affect output
                    ...
                    y_pred = exp_grad_red_pred.labels.ravel()
                ...
                ## compute multiplicity from y_pred_list 
                ave_multiplicity = self.compute_ave_multiplicity_helper(y_pred_list)
                sub_results['multiplicity'].append(ave_multiplicity)

                if constraint == 'EqualizedOdds':
                    self.print_subresults(sub_results, ['avgeo', 'acc', 'multiplicity', 'time'])
                else: 
                    self.print_subresults(sub_results, ['sp', 'acc', 'multiplicity', 'time'])
    ...
    def eqodds(self, model_name='gbm', num_iter=10, rand_seed=25, calibrated=False, constraint='weighted'): 
        metric_names = ['sp', 'avgeo', 'abseo', 'maxeo', 'acc', 'brier', 'multiplicity', 'time']
    ...
    def roc(self, model_name='gbm', num_iter=10, rand_seed=25, constraint='EqualizedOdds', params=[0.1]): 
