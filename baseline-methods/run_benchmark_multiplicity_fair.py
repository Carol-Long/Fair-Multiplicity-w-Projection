...
from benchmark_multiplicity_fair import Benchmark_Multiplicity_Fair
...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
            print('Correct Usage:\n')
            print('python run_benchmark_multiplicity_fair.py -m [model name] -f [fair method] -c [constraint] -n [num iter] -i [inputfile] -s [seed]')
            print('\n')
            ...
            print('[seed]: Any integer (Default: 25)')
            print('\n')
            sys.exit()
        elif opt == "-m":
            model = arg
    ...
    bm = Benchmark_Multiplicity_Fair(df, privileged_groups, unprivileged_groups, protected_attrs,label_name)
